...
def get_custom_patient_data(patient, norm=True, sample_plot=False):
    """
    Assumes that all folder called mit_data is next folder 
    in current directory. Can change this function internally 
    or write your own personalized one. 
    ...
    #patient=str(patient)
    ecg_notes= pd.read_fwf('Test/{}_annotations.txt'.format(patient),widths=widths).drop(['Unnamed: 0'],axis=1)
    ecg_data= pd.read_csv('Test/{}.csv'.format(patient))
    ecg_data.columns= ['samp_num','signal','V']
    ecg_notes=ecg_notes[['Sample #','Type','Aux']]
    ecg_notes.columns=['sample_num','type','aux']
    if norm == True:
    ...
    if sample_plot == True:
        ...
        plt.xlim(b,b+5000)
        plt.plot(peaklist, ecg_data.signal[peaklist], "x")
        plt.title(' Patient {} data'.format(patient))
        return None
        
    return ecg_data.signal,ecg_notes


def isolate_custom_patient_data(patient,classes,classes_further,classes_reducer=None, min_HR= 40,max_HR= 140,fs=360,verbose=False,plot_figs=True):
    """
    Isolation Model. Examines Patients, Normalizes Signal,
    and creates a python array with a length of the number of heat
    beates by the lenght of the longest heart rate signal. Signals
    ...
    ecg_signal,ecg_notes= get_custom_patient_data(patient)
    peaklist= ecg_notes.sample_num.astype(int).values 
    for c in classes.values():
        class_loc=[]
        if classes_reducer != None:
...
def plot_ecg(ecg, pred, norm=True, sample_plot=False):
    """
    Assumes that all folder called mit_data is next folder 
    in current directory. Can change this function internally 
    or write your own personalized one. 
    ...
    #patient=str(patient)
    #ecg_notes = [[i+1] + row for i, row in enumerate(pred)]
    ecg_notes = pd.DataFrame([[i+1] + row for i, row in enumerate(pred)], columns=['sample_num', 'type'])
    #ecg_notes= pd.read_fwf('Test/{}_annotations.txt'.format(patient),widths=widths).drop(['Unnamed: 0'],axis=1)
    ecg_data= pd.read_csv('Test/{}.csv'.format(patient))
    ecg_data.columns= ['samp_num','signal','V']
    ecg_notes=ecg_notes[['Sample #','Type']]
    ecg_notes.columns=['sample_num','type']
    if norm == True:
        ecg_data.signal= z_norm(ecg_data.signal)
    if sample_plot == True:
        peaklist= pred
